...
router.get("/list", async (req, res) => {
  try {
    ...
    if (!notes) {
      return res
        .status(400)
        .json({ success: "false", error: "No notes found" });
    }
...
router.post("/add", async (req, res) => {
  try {
    ...
    if (!addedNote) {
      return res
        .status(400)
        .json({ success: "false", error: "Error adding note" });
    }
...
router.delete("/delete", async (req, res) => {
  try {
    ...
    console.log("userId:", userId, "idToDelete:", idToDelete, req.body);
    if (!idToDelete) {
      return res.status(400).json({ success: false, error: "Incomplete Data" });
    }
